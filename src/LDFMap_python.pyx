""" A Python implementation of the Locally-Scaled Diffusion Map 
	Dimensionality Reduction Technique. Python version is for
	debugging. Uses random values for RMSD.
"""
...
cdef _calcEpsilon(xi, RMSD, possible_epsilons, cutoff):
...
def _calcMDS(xi, RMSD, possible_epsilons):
...
def _calcIntrinsicDim(sv): #sv = status vector
...
def _calcStatusVectors(eigenvals):
...
def _derivative(double[:] eigenvals, double[:] epsilons, long e):
...
def _calcMarkovMatrix(RMSD, epsilons, N):	
